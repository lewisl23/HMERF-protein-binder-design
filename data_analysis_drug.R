# Data analysis for MD simulation of Fn-3 domain bound to protein candidate
library(bio3d)
library(tidyverse)
library(Cairo)
...
for (data in 1:length(trajdcd)) {
  ...
  ca.inds <- atom.select(pdb, elety="CA", resno=58:249)
...
bending_percent_plot <- ggplot(percentage_angle_table, aes(x=runname, y=percentage), xlab("Run"), ylab("Percentage (%)")) + 
  geom_bar(stat = "identity")+
