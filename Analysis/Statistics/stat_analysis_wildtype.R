...
angle_table <- data.frame(matrix(ncol = 0, nrow = 99000))
rmsd_table <- data.frame(matrix(ncol = 0, nrow = 99000))
...
level = c("Wildtype+D1315_173", "Wildtype+D1315_37","Wildtype+D1295_60",
          "Wildtype+D1295_198", "Wildtype+D1063_37", "Wildtype+D1915_0")

#Set PDF size
CairoPDF("Stat_analysis_WT", width = 7, height = 7)


for (data in 1:length(angle_list)) {
  #Read the angle file and save it into dataframe
  ...
  histogram_angle <- hist(angle_data, xlim=c(60,180), ylim = c(0,35000), breaks = breaks,
                          main = paste("Bending angle of", subject, sep = " "),
                          xlab = "Angle (degree)")
  print(histogram_angle)
  
  #Plot a histogram for the bending angle for each variable
  histogram_rmsd <- hist(rmsd_data, xlim=c(0,15), ylim = c(0,35000),
...
rmsd_violin_table <- data.frame(group = rep(c("Wildtype+D1315_173", "Wildtype+D1315_37","Wildtype+D1295_60",
                                              "Wildtype+D1295_198", "Wildtype+D1063_37", "Wildtype+D1915_0"), each = 99000),
                                rmsd = c(rmsd_table$`Wildtype+D1315_173`, rmsd_table$`Wildtype+D1315_37`, rmsd_table$`Wildtype+D1295_60`,
                                         rmsd_table$`Wildtype+D1295_198`,rmsd_table$`Wildtype+D1063_37`, rmsd_table$`Wildtype+D1915_0`))
...
rmsd_violin_plot <- ggplot(rmsd_violin_table, aes(x=group, y=rmsd, fill=group)) +
  ...
  labs(title = "RMSD of Fn-3 119", x = "Group", y = "RMSD")+
...
angle_stat$subject <- factor(angle_stat$subject, 
                             levels = c("Wildtype+D1315_173", "Wildtype+D1315_37","Wildtype+D1295_60",
                                        "Wildtype+D1295_198", "Wildtype+D1063_37", "Wildtype+D1915_0"))
...
angle_bar <- ggplot(angle_stat, aes(x = subject, y = mean, fill = subject)) +
  ...
  labs(title = "Average bending angles of Titin Fn3-119 domain",
       x = "" ,  y = "Angle (degree)") +
...
bending_violin_table <- data.frame(group = rep(c("Wildtype+D1315_173", "Wildtype+D1315_37","Wildtype+D1295_60",
                                                 "Wildtype+D1295_198", "Wildtype+D1063_37", "Wildtype+D1915_0"), each = 99000),
                                   angle = c(angle_table$`Wildtype+D1315_173`, angle_table$`Wildtype+D1315_37`, angle_table$`Wildtype+D1295_60`,
                                             angle_table$`Wildtype+D1295_198`,angle_table$`Wildtype+D1063_37`, angle_table$`Wildtype+D1915_0`))
...
bending_violin_plot <- ggplot(bending_violin_table, aes(x=group, y=angle, fill=group)) +
  ...
  geom_boxplot(width = 0.1, outlier.shape = NA, show.legend = FALSE)+
  labs(title = "Bending angle of Fn-3 119", x = "Group", y = "Angle (degree)")+
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(angle = 45, hjust = 0.75, vjust = 0.8))
print(bending_violin_plot)

#Calculate bending angle into percentage
percentage_angle <- bending_angle_table / 99000*100

#Plot bar chart for bending angle percentage
percentage_angle_table <- data.frame(runname = level,
                                     percentage = c(percentage_angle$`Wildtype+D1315_173`, percentage_angle$`Wildtype+D1315_37`,
                                                    percentage_angle$`Wildtype+D1295_60`, percentage_angle$`Wildtype+D1295_198`,
                                                    percentage_angle$`Wildtype+D1063_37`, percentage_angle$`Wildtype+D1915_0`))
